            ...
            <form name="myForm" id="myForm" action="{{ route('rider.book') }}" method="post">
                <a class="bnr_a" href="#">
                    <div class="bnr_input_group1">
                        ...
                        <input type="text" class="bnr_input1 bnr_fa_input" required onkeypress="initialize()"
                            id="origin-input" name="origin-input" placeholder="Enter Pickup Location" autocomplete=""
                ...
                <a class="bnr_a" href="#">
                    <div class="bnr_input_group1">
                        ...
                        <input type="text" class="bnr_input1 bnr_fa_input" required onkeypress="initialize1()"
                            id="destination-input" name="destination-input" placeholder="Enter Drop Location"
                ...
                <div class="bnr_input_group1" id="ifYes11">
                    ...
                    <span class="bnr_input1">
                        <select class="bnr_select1" id="sel1" name="sel1"
                            onchange="yesnoCheck(this);update(this.value);">
                ...
                <div id="ifYes" style="display: none; margin-bottom: 5px;">
                    <div class="bnr_input_group1">
                        <input type="text" class="bnr_select2_1" name="date" id="datepicker" />
                        <?php
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        <div class="col-md-4 pl-0">
                            <input type="button" value="Get A Fare" class="btn btn-book" onclick="GetRoute()"
                                style="">
                        </div>
                        <div class="col-md-4">
...
@section('script')
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ config('services.googlemap.apikey') }}&map_ids=af9935eed520f3ec&libraries=places">
    </script>
    ...
    <script>
        ...
        function GetRoute() {
            ...
            service.getDistanceMatrix({
                ...
                if (status == google.maps.DistanceMatrixStatus.OK && response.rows[0].elements[0].status !=
                    ...
                    document.getElementById('fare_distance').innerHTML = "distance :  " + distance + "<br>" +
                    ...
                    for (i = 0; i < service_types.length; i++) {
                        ...
                        str += '<li class="row" style="padding:2px;width:100%;">' +
                            ...
                            '<span style="font-size: 18px; line-height: 40px;">' + service_types[i].name +
                            '</span>' +
                            ...
                            '<span style="font-size:20px;"><a data-toggle="modal" data-target="#farecard-modal-' +
                            service_types[i].id + '"><i class="fa fa-info-circle"></i></a></span>' +
                        ...
                        str += '<div id="farecard-modal-' + service_types[i].id +
                            ...
                            '<div class="col-md-6 text-right">A$' + parseFloat(service_types[i]
                                .base_ride_distance_charge).toFixed(2) + '</div>' +
                            '</div>' +
                            '<div class="row">' +
                            '<div class="col-md-6 text-left">Minimum fare</div>' +
                            '<div class="col-md-6 text-right">A$' + parseFloat(service_types[i].minimum_fare)
                            .toFixed(2) + '</div>' +
                            '</div>' +
                            '<div class="row">' +
                            '<div class="col-md-6 text-left">Per-km</div>' +
                            '<div class="col-md-6 text-right">A$' + parseFloat(service_types[i].price_per_unit)
                            .toFixed(2) + '</div>' +
                            '</div>' +
                            '<div class="row">' +
                            '<div class="col-md-6 text-left">Per-minute</div>' +
                            '<div class="col-md-6 text-right">A$' + parseFloat(service_types[i].price_per_minute)
                            .toFixed(2) + '</div>' +
                            '</div>' +
                            '<div class="row">' +
                            '<div class="col-md-6 text-left">Estimated Surcharges</div>' +
                            '<div class="col-md-6 text-right">A$' + parseFloat(estimated_surchages).toFixed(2) +
                            '</div>' +
                            '</div>';
                        str +=
                            '<div class="row">' +
                            '<div class="col-md-6 text-left">Booking fee</div>' +
                            '<div class="col-md-6 text-right">A$' + parseFloat(service_types[i].booking_charge)
                            .toFixed(2) + '</div>' +
                            '</div>' +
                            '</div>' +
                            '</div>' +
                            ...
                            '<div class="row">' +
                            '<div class="col-md-3">' +
                            '<button class="btn btn-primary">Request ' + service_types[i].name +
        ...
        function totalEstimate(fare, km, minute) {
            ...
            var baby_seat_charge = fare.baby_seat_charge === null ? 0 : parseFloat(fare.baby_seat_charge);
            var pet_charge = fare.pet_charge === null ? 0 : parseFloat(fare.pet_charge);
            var subtotal = base_cost_km + base_cost_minute + base_cost + minimum_fare + nsw_gtl_charge + nsw_ctp_charge +
                fuel_charge + booking_charge + baby_seat_charge + pet_charge;
