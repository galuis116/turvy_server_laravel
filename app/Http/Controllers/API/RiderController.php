...
class RiderController extends Controller
{
    ...
    public function addPaytoken(Request $request)
    {
        ...
        if (!$tokens) {
            ...
            $PaymentTokens->payment_method = $request->payment_method;
            $PaymentTokens->customer_id = $request->customer_id;
    ...
    public function deleteAccount(Request $request)
    {
        ...
        if ($res) {
            ...
            return response()->json([
                'status' => 0,
                'message' => 'Rider account not deleted',
                'datetime' => date('Y-m-d H:i'),
                'data' => null
            ]);
        }
    ...
    // Delete account status check 
    public function deleteAccountCheck(Request $request)
    {
        $data = $request->all();
        if (isset($data['phone'])) {
            $RiderRec = User::where('mobile', $data['phone'])->where('status', 0)->where('is_delete_request', 1)->first();
            if ($RiderRec && $RiderRec->id) {
                return response()->json([
                    'status' => 1,
                    'message' => $data['phone'] . ' Rider account deletion in process!',
                    'datetime' => date('Y-m-d H:i'),
                    'data' => null
                ]);
            } else {
                return response()->json([
                    'status' => 0,
                    'message' => 'Rider account not exist or not deleted procceed next.',
                    'datetime' => date('Y-m-d H:i'),
                    'data' => null
                ]);
            }
        ...
        return response()->json([
            'status' => 0,
            'message' => 'Rider account not exist procceed next.',
            'datetime' => date('Y-m-d H:i'),
            'data' => $data['phone']
        ]);
