  ...
  public function getdriverVeh($driver_id = 0){
    ...
    */
    ...
    public function deleteAccount(Request $request){
        ...
        $res = $driver->update();
         ...
         if($res){
            ...
            return response()->json([
                'status' => 0,
                'message' => 'Driver account not deleted',
                'datetime' => date('Y-m-d H:i'),
                'data' => null
            ]);
        }
    ...
    public function deleteAccountCheck(Request $request){
        $data = $request->all();
        if(isset($data['phone'])){
            $driverRec = Driver::where('mobile',$data['phone'])->where('is_approved',0)->where('is_delete_request',1)->first();
            if($driverRec && $driverRec->id){
                  return response()->json([
                    'status' => 1,
                    'message' => $data['phone'].' Driver account deletion in process!',
                    'datetime' => date('Y-m-d H:i'), 
                    'data' => null
                ]);
            }else{
                  return response()->json([
                    'status' => 0,
                    'message' => 'Driver account not exist or not deleted procceed next.',
                    'datetime' => date('Y-m-d H:i'), 
                    'data' => null
                ]);
            }
        ...
        return response()->json([
                'status' => 0,
                'message' => 'Driver account not exist procceed next.',
                'datetime' => date('Y-m-d H:i'), 
                'data' => $data['phone']
            ]);
